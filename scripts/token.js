...
function newToken(username) {
  ...
  var tokenToEnter = JSON.parse(`{
    ...
    "phone": "0000000000",
    "token": "example_token",
    "created": "2024-02-21 12:00:00",
    "expiry": "2024-03-13 12:00:00",
    "confirmed": "tbd",
    "status": "valid"
}`);
  ...
  if (fs.existsSync("./scripts/json/tokens.json")) {
    fs.readFile(__dirname + "/json/tokens.json", "utf-8", (error, data) => {
      ...
      if (!userMatch) {
        ...
        if (DEBUG) console.log("Error - User name is already taken");
        myEmitter.emit(
          "log",
          "token.newToken()",
          "ERROR",
          `username: ${username} is taken, failed to create new token.`
        );
      }
    });
  } else {
          ...
          if (obj.username === Args[3]) {
            ...
            switch (Args[2]) {
              ...
              case "EMAIL":
                ...
                break;
              default:
                if (DEBUG) console.log(`unknown command ${Args[2]} was passed`);
                console.log(`try --token [username] [phone or email]`);
                myEmitter.emit(
                  "log",
                  "token.updateToken()",
                  "ERROR",
                  ` unknown command ${Args[2]} was passed.`
                );
            }
            if (DEBUG) console.log(obj);
          }
...
function checkExpiryDate() {
  if (DEBUG) console.log("token.checkExpiryDate(): started\n");
  if (DEBUG) console.log(Args);
